...
# Algorithm Learning Maximum Margin Methods                        #
...
cvsvmtb = cv.GridSearchCV(ml.SVR(kernel='rbf'), cv=cv.TimeSeriesSplit(n_splits=10),
                          param_grid={"C": [0.25, 0.50, 1.00, 1.25]}).fit(pcat, rspyt['rspy'])
...
print("")
print("RBF Support Vector Machine Regression A Optimal Error Term Penalty: ", cvsvmpara)
cvsvmparb = cvsvmtb.best_estimator_.C
print("RBF Support Vector Machine Regression B Optimal Error Term Penalty: ", cvsvmparb)
print("")

# 5.3. Maximum Margin Method Algorithm Training
svmta = ml.SVR(kernel='rbf', C=cvsvmpara).fit(rspyt[pfa], rspyt['rspy'])
svmtb = ml.SVR(kernel='rbf', C=cvsvmparb).fit(pcat, rspyt['rspy'])

# 5.4. Maximum Margin Method Algorithm Testing
svmfa = svmta.predict(rspyf[pfa])
svmfb = svmtb.predict(pcaf)

# 5.4.1. Algorithm Testing Charts

# RBF Support Vector Machine Regression A Testing Chart
svmfadf = pd.DataFrame(svmfa, index=rspyf.index)
fig1, ax = plt.subplots()
ax.plot(rspyf['rspy'])
ax.plot(svmfadf, label='svmfa')
plt.legend(loc='upper left')
plt.title('RBF Support Vector Machine Regression A Testing Chart')
plt.ylabel('Log Returns')
plt.xlabel('Date')
plt.show()

# RBF Support Vector Machine Regression B Testing Chart
svmfbdf = pd.DataFrame(svmfb, index=rspyf.index)
fig2, ax = plt.subplots()
ax.plot(rspyf['rspy'])
ax.plot(svmfbdf, label='svmfb')
plt.legend(loc='upper left')
plt.title('RBF Support Vector Machine Regression B Testing Chart')
plt.ylabel('Log Returns')
plt.xlabel('Date')
plt.show()

# 5.5. Maximum Margin Method Algorithm Testing Forecasting Accuracy
svmmaea = fa.mean_absolute_error(rspyf['rspy'], svmfa)
svmmaeb = fa.mean_absolute_error(rspyf['rspy'], svmfb)
svmmsea = fa.mean_squared_error(rspyf['rspy'], svmfa)
svmmseb = fa.mean_squared_error(rspyf['rspy'], svmfb)
svmrmsea = np.sqrt(svmmsea)
svmrmseb = np.sqrt(svmmseb)
print("== Maximum Margin Method Algorithm Testing Forecasting Accuracy ==")
print("")
print("Mean Absolute Error ", "A:", round(svmmaea, 6), "B:", round(svmmaeb, 6))
print("Mean Squared Error ", "A:", round(svmmsea, 6), "B:", round(svmmseb, 6))
print("Root Mean Squared Error ", "A:", round(svmrmsea, 6), "B:", round(svmrmseb, 6))
